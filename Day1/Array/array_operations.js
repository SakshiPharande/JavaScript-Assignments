...
function filterByName(str) {
    return arr.filter(person => person.first_name.toLowerCase().includes(str.toLowerCase()));
}
console.log(filterByName("v")); // Returns objects with "Virgilio" and "Venita"


const emails = arr.map(person => person.email);
console.log(emails);

const sortedByDOB = [...arr].sort((a, b) => new Date(b.date_of_birth) - new Date(a.date_of_birth));
